...
var NzCarouselOpacityStrategy = class extends NzCarouselBaseStrategy {
  ...
  switch(_f, _t) {
    ...
    const complete$ = new import_rxjs.Subject();
    this.contents.forEach((content, i) => {
...
var NzCarouselComponent = class _NzCarouselComponent {
  ...
  ngOnInit() {
    ...
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs.fromEvent)(this.slickListEl, "keydown").pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
        const {
          keyCode
        } = event;
    ...
    this.nzResizeObserver.observe(this.el).pipe((0, import_operators.debounceTime)(100), (0, import_operators.distinctUntilChanged)(), (0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      this.layout();
    });
  }
  ngAfterContentInit() {
...
var NzCarouselFlipStrategy = class extends NzCarouselBaseStrategy {
  withCarouselContents(contents) {
    ...
    if (this.contents) {
      ...
      carouselComponent.ngZone.runOutsideAngular(() => {
        (0, import_rxjs.timer)(carouselComponent.nzTransitionSpeed).subscribe(() => {
  ...
  switch(rawF, rawT) {
    ...
    (0, import_rxjs.timer)(speed).subscribe(() => {
      complete$.next();
      complete$.complete();
    });
    if (rawF === rawT) {
