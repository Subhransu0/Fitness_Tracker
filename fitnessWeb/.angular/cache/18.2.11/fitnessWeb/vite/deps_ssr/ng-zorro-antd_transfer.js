...
var NzTransferListComponent = class _NzTransferListComponent {
  ...
  ngAfterViewInit() {
    this.checkboxes.changes.pipe((0, import_operators.startWith)(this.checkboxes), (0, import_operators.switchMap)(() => {
      const checkboxes = this.checkboxes.toArray();
      return new import_rxjs.Observable((subscriber) => this.ngZone.runOutsideAngular(() => (0, import_rxjs.merge)(...checkboxes.map((checkbox) => (0, import_rxjs.fromEvent)(checkbox.nativeElement, "click"))).subscribe(subscriber)));
    })).subscribe((event) => {
...
var NzTransferComponent = class _NzTransferComponent {
  ...
  constructor(cdr, i18n, elementRef, renderer, directionality) {
    ...
    this.nzCanMove = (arg) => (0, import_rxjs.of)(arg.list);
  ...
  ngOnInit() {
    this.nzFormStatusService?.formStatusChanges.pipe((0, import_operators.distinctUntilChanged)((pre, cur) => {
      ...
      hasFeedback
    })), (0, import_operators.takeUntil)(this.unsubscribe$)).subscribe(({
      status,
      hasFeedback
    }) => {
      this.setStatusStyles(status, hasFeedback);
    });
    this.i18n.localeChange.pipe((0, import_operators.takeUntil)(this.unsubscribe$)).subscribe(() => {
      this.locale = this.i18n.getLocaleData("Transfer");
      this.markForCheckAllList();
    });
    this.dir = this.directionality.value;
    this.directionality.change?.pipe((0, import_operators.takeUntil)(this.unsubscribe$)).subscribe((direction) => {
      this.dir = direction;
      this.cdr.detectChanges();
    });
  }
