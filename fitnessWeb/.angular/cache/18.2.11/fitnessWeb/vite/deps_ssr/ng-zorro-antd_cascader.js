...
var NzCascaderService = class _NzCascaderService {
  constructor() {
    ...
    this.values = [];
    this.$loading = new import_rxjs.BehaviorSubject(false);
    this.$redraw = new import_rxjs.Subject();
    this.$optionSelected = new import_rxjs.Subject();
    this.$quitSearching = new import_rxjs.Subject();
  ...
  loadChildren(option, columnIndex, success, failure) {
    ...
    if (loadFn) {
      ...
      (0, import_rxjs.from)(loadFn(option, columnIndex)).pipe((0, import_operators.finalize)(() => {
...
var NzCascaderComponent = class _NzCascaderComponent {
  ...
  constructor(cascaderService, nzConfigService, ngZone, cdr, i18nService, destroy$, elementRef, renderer, directionality) {
    ...
    this.input$ = new import_rxjs.BehaviorSubject(void 0);
  ...
  ngOnInit() {
    ...
    srv.$redraw.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      this.checkChildren();
    ...
    srv.$loading.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((loading) => {
      this.isLoading = loading;
    });
    srv.$optionSelected.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((data) => {
      if (!data) {
        this.onChange([]);
    ...
    srv.$quitSearching.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      this.inputString = "";
      this.dropdownWidthStyle = "";
    });
    this.i18nService.localeChange.pipe((0, import_operators.startWith)(), (0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      this.setLocale();
    });
    ...
    this.directionality.change.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      this.dir = this.directionality.value;
      srv.$redraw.next();
    });
  ...
  setupChangeListener() {
    this.input$.pipe((0, import_operators.switchMap)((input) => input ? new import_rxjs.Observable((subscriber) => this.ngZone.runOutsideAngular(() => (0, import_rxjs.fromEvent)(input.nativeElement, "change").subscribe(subscriber))) : import_rxjs.EMPTY), (0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => event.stopPropagation());
  }
  setupKeydownListener() {
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs.fromEvent)(this.el, "keydown").pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
