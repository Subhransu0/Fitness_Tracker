...
var NzResizableService = class _NzResizableService {
  constructor(ngZone) {
    ...
    this.handleMouseDownOutsideAngular$ = new import_rxjs.Subject();
    this.documentMouseUpOutsideAngular$ = new import_rxjs.Subject();
    this.documentMouseMoveOutsideAngular$ = new import_rxjs.Subject();
    this.mouseEnteredOutsideAngular$ = new import_rxjs.Subject();
  }
  startResizing(event) {
...
var NzResizableDirective = class _NzResizableDirective {
  constructor(elementRef, renderer, nzResizableService, platform, ngZone, destroy$) {
    ...
    this.sizeCache = null;
    this.nzResizableService.handleMouseDownOutsideAngular$.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
      if (this.nzDisabled) {
        return;
      }
      this.resizing = true;
    ...
    this.nzResizableService.documentMouseUpOutsideAngular$.pipe((0, import_operators.takeUntil)(this.destroy$), (0, import_operators.filter)(Boolean)).subscribe((event) => {
      if (this.resizing) {
        this.resizing = false;
    ...
    this.nzResizableService.documentMouseMoveOutsideAngular$.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
      if (this.resizing) {
        this.resize(event);
      }
    });
  ...
  ngAfterViewInit() {
    ...
    this.setPosition();
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs.fromEvent)(this.el, "mouseenter").pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
        this.nzResizableService.mouseEnteredOutsideAngular$.next(true);
      });
      (0, import_rxjs.fromEvent)(this.el, "mouseleave").pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
...
var NzResizeHandleComponent = class _NzResizeHandleComponent {
  ...
  ngOnInit() {
    this.nzResizableService.mouseEnteredOutsideAngular$.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((entered) => {
      if (entered) {
    ...
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs.merge)((0, import_rxjs.fromEvent)(this.host.nativeElement, "mousedown", passiveEventListenerOptions), (0, import_rxjs.fromEvent)(this.host.nativeElement, "touchstart", passiveEventListenerOptions)).pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
