...
var NzRateComponent = class _NzRateComponent {
  ...
  ngOnInit() {
    ...
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs.fromEvent)(this.ulElement.nativeElement, "focus").pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
        this.isFocused = true;
        if (this.nzOnFocus.observers.length) {
          this.ngZone.run(() => this.nzOnFocus.emit(event));
        }
      });
      (0, import_rxjs.fromEvent)(this.ulElement.nativeElement, "blur").pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
        this.isFocused = false;
