...
var TimelineService = class _TimelineService {
  constructor() {
    this.check$ = new import_rxjs.ReplaySubject(1);
  }
  markForCheck() {
    this.check$.next();
  }
...
var NzTimelineComponent = class _NzTimelineComponent {
  ...
  ngOnInit() {
    this.timelineService.check$.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
  ...
  ngAfterContentInit() {
    this.updateChildren();
    this.listOfItems.changes.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      this.updateChildren();
    });
  }
  ngOnDestroy() {
