...
var NzAutocompleteOptionComponent = class _NzAutocompleteOptionComponent {
  ...
  ngOnInit() {
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs2.fromEvent)(this.element.nativeElement, "mouseenter").pipe((0, import_operators.filter)(() => this.mouseEntered.observers.length > 0), (0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
        this.ngZone.run(() => this.mouseEntered.emit(this));
      });
      (0, import_rxjs2.fromEvent)(this.element.nativeElement, "mousedown").pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => event.preventDefault());
    });
  }
  ngOnDestroy() {
    this.destroy$.next();
...
var NzAutocompleteTriggerDirective = class _NzAutocompleteTriggerDirective {
  ...
  ngAfterViewInit() {
    if (this.nzAutocomplete) {
      this.nzAutocomplete.animationStateChange.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
  ...
  subscribeOptionsChange() {
    const optionChanges = this.nzAutocomplete.options.changes.pipe((0, import_operators.tap)(() => this.positionStrategy.reapplyLastPosition()), (0, import_operators.delay)(0));
  ...
  subscribeOverlayOutsideClick() {
    return this.overlayRef.outsidePointerEvents().pipe((0, import_operators.filter)((e) => !this.elementRef.nativeElement.contains(e.target))).subscribe(() => {
      this.closePanel();
    });
  }
  attachOverlay() {
    ...
    if (this.overlayRef && !this.overlayRef.hasAttached()) {
      ...
      this.overlayRef.detachments().pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
        this.closePanel();
      });
    }
...
var NzAutocompleteComponent = class _NzAutocompleteComponent {
  ...
  constructor(changeDetectorRef, directionality) {
    ...
    this.selectionChangeSubscription = import_rxjs2.Subscription.EMPTY;
    this.optionMouseEnterSubscription = import_rxjs2.Subscription.EMPTY;
    this.dataSourceChangeSubscription = import_rxjs2.Subscription.EMPTY;
    this.optionSelectionChanges = (0, import_rxjs2.defer)(() => {
      if (this.options) {
        return (0, import_rxjs2.merge)(...this.options.map((option) => option.selectionChange));
      }
      return this.afterNextRender$.pipe((0, import_operators.switchMap)(() => this.optionSelectionChanges));
    });
    this.optionMouseEnter = (0, import_rxjs2.defer)(() => {
      if (this.options) {
        return (0, import_rxjs2.merge)(...this.options.map((option) => option.mouseEntered));
      }
      return this.afterNextRender$.pipe((0, import_operators.switchMap)(() => this.optionMouseEnter));
    });
  ...
  subscribeOptionChanges() {
    this.selectionChangeSubscription.unsubscribe();
    this.selectionChangeSubscription = this.optionSelectionChanges.pipe((0, import_operators.filter)((event) => event.isUserInput)).subscribe((event) => {
      event.source.select();
