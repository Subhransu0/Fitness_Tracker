...
var NzStepComponent = class _NzStepComponent {
  ...
  constructor(cdr, ngZone, destroy$) {
    ...
    this.clickOutsideAngular$ = new import_rxjs.Subject();
    this.nullProcessFormat = () => null;
    this._currentIndex = 0;
  }
  ngOnInit() {
    this.ngZone.runOutsideAngular(() => (0, import_rxjs.fromEvent)(this.itemContainer.nativeElement, "click").pipe((0, import_operators.filter)(() => this.clickable && this.currentIndex !== this.index && !this.nzDisabled), (0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
...
var NzStepsComponent = class _NzStepsComponent {
  ...
  constructor(ngZone, cdr, directionality, destroy$) {
    ...
    this.indexChangeSubscription = import_rxjs.Subscription.EMPTY;
    this.showProcessDot = false;
  ...
  ngAfterContentInit() {
    if (this.steps) {
      this.steps.changes.pipe((0, import_operators.startWith)(null), (0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
  ...
  updateChildrenSteps() {
    if (this.steps) {
      ...
      this.indexChangeSubscription = (0, import_rxjs.merge)(...this.steps.map((step) => step.clickOutsideAngular$)).pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((index) => {
