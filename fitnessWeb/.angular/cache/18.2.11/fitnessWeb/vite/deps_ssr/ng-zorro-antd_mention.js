...
var NzMentionService = class _NzMentionService {
  constructor() {
    this.triggerChange$ = new import_rxjs.Subject();
  }
  triggerChanged() {
...
var NzMentionTriggerDirective = class _NzMentionTriggerDirective {
  ...
  setupEventListener(eventName, eventEmitter, shouldPassEvent = false) {
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs.fromEvent)(this.el.nativeElement, eventName).pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe((event) => {
...
var NzMentionComponent = class _NzMentionComponent {
  ...
  ngAfterViewInit() {
    this.items.changes.pipe((0, import_operators.startWith)(this.items), (0, import_operators.switchMap)(() => {
      const items = this.items.toArray();
      return new import_rxjs.Observable((subscriber) => this.ngZone.runOutsideAngular(() => (0, import_rxjs.merge)(...items.map((item) => (0, import_rxjs.fromEvent)(item.nativeElement, "mousedown"))).subscribe(subscriber)));
    })).subscribe((event) => {
  ...
  subscribeOverlayOutsideClick() {
    ...
    subscription.add(this.ngZone.runOutsideAngular(() => (0, import_rxjs.fromEvent)(this.document, "touchend").subscribe((event) => canCloseDropdown(event) && this.ngZone.run(() => this.closeDropdown()))));
    return subscription;
  }
  attachOverlay() {
    if (!this.overlayRef) {
